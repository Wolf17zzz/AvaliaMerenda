                    ...
                    <div class=" rounded mt-4">
                        <form method="POST" action="{{ route('avaliacao.store') }}">
                            ...
                            <div class="campo-comentario">
                                ...
                                <textarea class="form-control" id="avaliacao_{{ $index }}" name="comentario"
                                    placeholder="Comente aqui o que você achou da Merenda!"></textarea>
                                <div class="emoji-list">
                                    <span class="emoji" id="emoji1_{{ $index }}"
                                        onclick="insertEmoji('😃', {{ $index }})">😃</span>
                                    <span class="emoji" id="emoji2_{{ $index }}"
                                        onclick="insertEmoji('😋', {{ $index }})">😋</span>
                                    <span class="emoji" id="emoji3_{{ $index }}"
                                        onclick="insertEmoji('🙃', {{ $index }})">🙃</span>
                                    <span class="emoji" id="emoji4_{{ $index }}"
                                        onclick="insertEmoji('🤩', {{ $index }})">🤩</span>
                                    <span class="emoji" id="emoji5_{{ $index }}"
                                        onclick="insertEmoji('🤤', {{ $index }})">🤤</span>
                                    <span class="emoji" id="emoji6_{{ $index }}"
                                        onclick="insertEmoji('👍', {{ $index }})">👍</span>
                                    <span class="emoji" id="emoji7_{{ $index }}"
                                        onclick="insertEmoji('👎', {{ $index }})">👎</span>
